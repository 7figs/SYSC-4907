#!/usr/bin/env python3
"""
Sweep N_TRAIN from 1..100 and collect the same aggregate stats that the original
movie_tree_eval.py prints at the end (Aggregate pooled across users + Average across users),
then generate matplotlib scatterplots of each metric vs N.

Assumptions:
- This script lives in the same directory as:
    preferences.csv
    top500_movies_features.csv
- Preferences CSV format:
    movie_id, user_1, user_2, ... (each user column is 0/1)
- Features CSV:
    Rows ordered by movie_id 1..500 (or has movie_id column)
...
M_RUNS = 100                 # number of random splits per user per N
...
def seed_for_n(base_seed: int | None, n_train: int) -> int | None:
    """
    If RANDOM_SEED is set, make results deterministic per N by deriving a seed from it.
    If RANDOM_SEED is None, keep None (fully random).
    """
...
def evaluate_for_n(
    ...
    """
    Returns the "end of script" aggregate metrics for a single N:
      - aggregate pooled across all users: accuracy/recall/precision/fpr + confusion counts + pooled p@k
      - average across users: avg accuracy/recall/precision/fpr + avg p@k
    """
    ...
    for user_col in label_cols:
        ...
        for _ in range(m_runs):
            ...
            p, k_eff, _ = precision_at_k_user_vector(
                X_all_np=X_all_np,
                y_all=y,
                train_idx=train_idx,
                k=P_AT_K,
    ...
    return {
        ...
        # Aggregate (pooled across all users)
...
def main() -> None:
    ...
    # Choose numeric feature columns (ignore text)
    ...
    # Clamp N sweep to dataset constraints
    n_min = max(N_MIN, 1)
    n_max = min(N_MAX, len(df) - 1)
    if n_min > n_max:
        raise ValueError(f"Invalid sweep bounds after clamping: {n_min}..{n_max} (dataset rows={len(df)})")
    ...
    # Confusion counts to plot (ints, but plotted as float scatter)
    count_metrics = [
        ("agg_tn", "Aggregate True Negatives (pooled)"),
        ("agg_fp", "Aggregate False Positives (pooled)"),
        ("agg_fn", "Aggregate False Negatives (pooled)"),
        ("agg_tp", "Aggregate True Positives (pooled)"),
    ]
    for key, nice in count_metrics:
        ys = [float(r[key]) for r in results]
        save_scatter(
            xs=xs,
            ys=ys,
